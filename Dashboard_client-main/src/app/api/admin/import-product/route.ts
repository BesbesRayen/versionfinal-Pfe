...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await ProductImporterService.importFromUrl(body.url, { storeId: body.storeId });
    recordImportLog(result);
    return NextResponse.json(result);
  } catch {
    return NextResponse.json({ error: 'Impossible de traiter cette URL.' }, { status: 500 });
  }
}
